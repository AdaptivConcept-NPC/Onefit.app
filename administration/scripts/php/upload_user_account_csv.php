    ...
    try {
        ...
        if (!empty($_FILES['csvfile-user_accounts']['name']) && in_array($_FILES['csvfile-user_accounts']['type'], $fileMimes)) {
            ...
            while (($getData = fgetcsv($csvFile, 10000, ",")) !== FALSE) {
                ...
                $username = sanitizeMySQL($dbconn, $usrnm); //$getData[1]);
                $password_hash = sanitizeMySQL($dbconn, $pwdhash);
                ...
                $query = "SELECT * FROM `users` WHERE `user_email` = '$user_email' AND `user_name`='$user_name' AND `user_surname`='$user_surname'";
                ...
                if ($check->num_rows > 0) {
                    ...
                    VALUES 
                    (null, '$username', '$password_hash', '$user_name', '$user_surname', '$id_number', '$user_email', '$contact_number', '$date_of_birth', '$user_gender', '$user_race', '$user_nationality', $account_active)");

                    // $result = $dbconn->query($query);

                    if (!$result) die("An error occurred while trying to save the new record [ user_id: $user_id | username: $username | user_email: $user_email | user_name: $user_name | user_surname: $user_surname ]: " . $dbconn->error . "]");
                }
            }
