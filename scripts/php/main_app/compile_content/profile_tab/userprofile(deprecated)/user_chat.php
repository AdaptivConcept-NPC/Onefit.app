...
if (isset($_SESSION["currentUserAuth"])) {
  ...
  if (isset($_GET['cref'])) {
    $conversation_ref = sanitizeString($_GET['cref']);
  } else {
...
function getUserChats($conversation_ref)
{
  ...
  $conversation_ref = $conversation_ref || $conversation_ref = "CNV_73HJD8690S"; // default conversation reference if null
  $currentUser_Usrnm = sanitizeString($_SESSION["currentUserUsername"]);
  $profile_pic = sanitizeString($_SESSION["currentUserProfilePic"]);
