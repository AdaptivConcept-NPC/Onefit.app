...
require("./compile_teams_day_activities_preview_column_bar.php");
require("./compile_teams_schedule_activity_submit_form.php");

//test connection - if fail then die
if ($dbconn->connect_error) die("Fatal Error");

// check if day and gref are set
if (isset($_GET['day']) && isset($_GET['gref'])) {
    ...
    // check the when param, if set then use it, else use this
    ...
    // echo "dayDateThisWeek: $dayDateThisWeek <br/>"; // debug
    ...
    if ($getGrpRef == "editbar") {
        ...
        try {
            ...
            if ($rows <= 0) {
                ...
                $preview_bar = compile_preview_column($dbconn);

                $form_html = compile_form($dbconn);
